...
namespace SEDrag.Definition
{
	public class HeatDefinition
	{
		...
		public void Init()
		{
			...
			HeatData value;
			Log.DebugWrite(DragSettings.DebugLevel.Info,"Writing heat defaults");
			data.Clear();
			#region defaults
				...
				try
				{
					...
					if (res.Length > 1)
					{
						...
						if(m_data.TryGetValue(keys.Id.SubtypeName, out value))
						{
							...
							Log.DebugWrite(DragSettings.DebugLevel.Info, string.Format("Removing duplicate match: {0}", keys.Id.SubtypeName));
						}
